      ...
      <div ref={ref} className="relative max-w-7xl mx-auto pb-20">
        {data.map((item, index) => (
          ...
          >
            ...
            <div className={`relative ${isRtl ? "pr-14 pl-4 md:pr-4" : "pl-14 pr-4 md:pl-4"} w-full`}>
              <h3 className={`md:hidden block text-2xl mb-4 ${isRtl ? "text-right" : "text-left"} font-bold text-neutral-500 dark:text-neutral-500`}>
                {item.title}
              </h3>
              {item.content}{" "}
            </div>
        ...
        >
          <motion.div
            ...
            className="absolute inset-x-0 top-0  w-[2px] bg-gradient-to-t from-[#FF6B35] via-[#F59E0B] to-transparent from-[0%] via-[10%] rounded-full"
          />
        </div>
      </div>
    </div>
